  ...
  return (
    <div className="min-h-screen bg-pine-800 text-cream">
      <div className="max-w-3xl mx-auto px-4 md:px-8 py-12">
        ...
        <a
          href="/"
          className="inline-flex items-center gap-2 text-cream/60 hover:text-amber-400 text-sm font-medium transition-colors mb-8"
        >
          <ArrowLeft className="w-4 h-4" />
          Back to main page
        </a>

        <h1 className="font-[family-name:var(--font-playfair)] text-3xl md:text-4xl font-bold text-amber-400 mb-2">
          Cost Breakdown
        </h1>
        <p className="text-cream/60 mb-8">
          Per-person estimates. Eric doesn&apos;t see this page.
        </p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-pine-700">
                <th className="text-left px-4 py-3 font-semibold text-amber-400">
                  Item
                </th>
          ...
          {costs
            ...
            .map((item) => (
              <p key={item.item} className="text-xs text-cream/50">
                <span className="text-cream/70 font-medium">{item.item}:</span>{" "}
                {item.notes}
              </p>
            ))}
        </div>
